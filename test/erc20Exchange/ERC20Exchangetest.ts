...
describe('ERC20Exchange test', () => {
  ...
  const setup = async (): Promise<ERC20ExchangeSetup> => {
    ...
    await exchange.addLiquidity(expectedSupply, {
      value: expectedAddedLiquidityEth,
    });
  ...
  describe('Add liquidity', () => {
    it('should revert', async () => {
      ...
      await expect(
        exchange
          .connect(user)
          .addLiquidity(0, { value: expectedAddedLiquidityEth }),
      ).to.be.reverted;

      await expect(
        exchange.addLiquidity(0, { value: expectedAddedLiquidityEth }),
      ).to.be.revertedWith('The inital supply must be a positive number');

      await expect(
        exchange.addLiquidity(expectedSupply, { value: 0 }),
    ...
    it('should update liquidity, balances, emit LiquidityChanged', async () => {
      ...
      const tx = await exchange.addLiquidity(expectedSupply, {
        value: expectedAddedLiquidityEth,
      });
      await expect(tx)
      ...
      const [exchangeLiquidityEthAfter, exchangeLiquidityTokenAfter] =
        await exchange.liquidity();

      expect(deployerEthBefore - deployerEthAfter).to.be.closeTo(
        expectedAddedLiquidityEth + gasCost,
        1e12,
      );
      expect(exchangeLiquidityEthAfter - exchangeLiquidityEthBefore).to.eq(
        expectedAddedLiquidityEth,
      );

      expect(exchangeLiquidityTokenAfter - exchangeLiquidityTokenBefore).to.eq(
        expectedSupply,
      );
    });
  });
  ...
  describe('Buy', () => {
    it("should update liquidity, user's eth and token balance, return true, emit Buy event", async () => {
      ...
      expect(
        await exchange.getFunction('buy').staticCall({ value: tradeEthAmount }),
      ).to.eq(true);
      ...
      expect(userEthBefore - tradeEthAmount).to.be.closeTo(
        userEthAfter + gasCost,
        12n,
      );
      expect(exchangeEthAfter - tradeEthAmount).to.eq(exchangeEthBefore);

      const tokens =
        (tradeEthAmount * 10n ** (await token.decimals())) / expectedPrice;
    ...
    it('should revert', async () => {
      ...
      await expect(
        exchange.connect(user).buy({ value: excessiveTradeEthAmount }),
      ).to.be.revertedWith(
  ...
  describe('Sell', () => {
    ...
    it('should first buy, then sell the bought amount, update the token, eth balance, emit Sell event', async () => {
      ...
      const userEthAfter = await hre.ethers.provider.getBalance(user);
      const [exchangeEthAfter, exchangeTokensAfter] =
        await exchange.liquidity();
      ...
      const tradedEth =
        (tokensSoldAfterFee * expectedPrice) / 10n ** (await token.decimals());
  ...
  describe('Weekly burn fee', () => {
    ...
    it('should revert', async () => {
      ...
      await expect(exchange.connect(user).weeklyBurnFee()).to.be.reverted;

      await expect(exchange.weeklyBurnFee()).to.be.revertedWith(
